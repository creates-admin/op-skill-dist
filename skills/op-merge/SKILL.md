...
### 0-channel-guard. mcp channel 未対応の明示停止 (ADR-0027 6b、第七波課題)

> **`OP_GITHUB_CHANNEL=mcp` では op-merge は未対応**。ADR-0027 6b で op-run 側 (post-check /
> global review / Review Fix Loop) は review state 文書 (`op review state pull/push`) 経由で
> mcp channel でも成立するようになったが、**op-merge の gate 判定はまだ mcp channel 向けの
> 素材化ができていない**: `mergeable_state` (gate 7-8) / CI checks (gate 9) の live 取得、
> confirmed head (stale 判定の基準となる「確定した現在 head」) の取得、および同時 merge の
> race 検出のいずれも mcp channel 向けの call-spec / 素材注入経路が未設計 (第七波の課題として
> 分離。理由は `op-tools/docs/adr/0027-review-state-body-document.md` 6b 実装記録節を参照)。
> op-merge 起動時、フェーズ0 で `OP_GITHUB_CHANNEL` を確認し、`mcp` の場合は以下を提示して
> **loud に停止**する (silent skip しない):
>
> ```text
> ❌ op-merge は OP_GITHUB_CHANNEL=mcp (Cloud) に未対応です。
>    理由: merge_state (gate 7-9) / confirmed head / 同時 merge の race 検出が
>          mcp channel 向けに素材化されていません (ADR-0027 第七波の課題)。
>    対応: ローカル (gh channel) セッションで op-merge を実行してください。
> ```
>
> ```bash
> if [ "${OP_GITHUB_CHANNEL:-gh}" = "mcp" ]; then
>   echo "❌ op-merge は OP_GITHUB_CHANNEL=mcp (Cloud) に未対応です (ADR-0027 第七波の課題)。" >&2
>   echo "   merge_state / confirmed head / race 検出が mcp channel 向けに素材化されていません。" >&2
>   echo "   ローカル (gh channel) セッションで op-merge を実行してください。" >&2
>   exit 1
> fi
> ```
