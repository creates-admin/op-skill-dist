...
| フェーズ | 役割 | 主要 CLI |
|---|---|---|
| 0 | 環境確認 + _shared 整合性 | (raw bash: git/gh precheck) |
| 1 | 対象 PR の選定 | `op pr list --label pro-reviewed` |
| 2 | PR 個別解説と確認 | `op pr view --include both` |
| 3 | マージ前最終ゲート (gate 1-21) | **`op merge verify`** (1 呼び出し) |
| 3.5 | MergeGateReport の人間向け解説 | (verify 出力の読み解き) |
| 3.6 | draft ready 化 + マージ実行 | `gh pr ready` + `op pr merge` |
| 4 | マージ後確認 + worktree cleanup | `op issue view` + `op run worktree cleanup` |
| 5 | 報告と次の PR へ | — |
...
`review-expert` (Phase 1 active) は op-run フェーズ4 で `<!-- op-review-meta -->` block を出力し、
op-merge はこれを gate 3〜5 の根拠とする。`pro-reviewed` 付与と `pro-review-needs-fix` /
...
- `<!-- op-review-meta -->` block が PR に存在しない場合、`op merge verify` は gate 3a で block を返す。
  op-merge はユーザーに「review-expert global review が未実行です。op-run を再実行するか手動レビューしますか?」を確認する
...
# git work tree 内であること + gh 認証済みであることを確認する。
...
```bash
# pro-reviewed の open PR 番号一覧を取得する (両モード共通)。
...
PR の `<!-- op-review-meta -->` block に含まれる `op_run_session_id` が自 `OP_RUN_SESSION_ID` と
一致する PR だけを対象とする。

`op_run_session_id` は **trusted author 投稿の op-review-meta コメント由来** の値を使う
(forge / 古いコメント混入を防ぐため、生 grep で抽出してはならない)。各 PR の `op merge verify`
出力 (`details.gates[]` の gate 3i 周辺、または `op pr view --include body-comments-commits` の
コメント走査) から session_id を取得して照合する。

> **落とし穴 (session filter)**: `op pr list` は session 概念を持たない (Issue A scope 外)。
> session 絞り込みは本 SKILL.md 側の後処理で行う。一覧取得 (`op pr list`) → 各 PR の
> review-meta から session_id 抽出 → 自 session と一致する PR 番号のみ残す、という 1 段の
...
- review-meta / post-check が stale (reviewed_head_sha != 現在 head) → 「stale」として別枠表示

> **重要**: 一覧表示段階で `gh pr view --jq '.comments[].body' | grep reviewed_head_sha` のような
> 生抽出をしてはならない。reviewed_head_sha / review 状態の正本は `op merge verify` の判定であり、
> 一覧でも verify 結果 (またはそれと同じ trusted-author / provenance 検証を経た値) を使う。
...
```bash
# PR の body + コメント + commit を 1 fetch で取得する。
...
#   --trusted-author: review-meta / post-check-meta の forge 防止に使う trusted author login (カンマ区切り)。
#                     省略すると fail-CLOSED (review_meta / post-check を 1 件も採用しない) ため必須に近い。
...
### 3-1. trusted author / TRUSTED_AUTHORS の解決方針
...
| gate 3a-3i (review-meta provenance) | 正式な op-run review を経ていない、または review コメントが偽装の疑いがあります |
...
if [ "$IS_DRAFT" = "true" ]; then
  ...
  gh pr ready "$PR_NUM"   # raw bash 残置: ready 化は op pr 系に未実装の低頻度 git/gh operation
fi
```

> **ready→merge の race condition**: ready 化直後に commit が積まれる稀なレースは、
> ready 化後すぐ `op merge verify` を **再度呼ぶ** ことで塞ぐ (gate 4 / 7 が
> `head_sha_inconsistent` を二度 fetch で検出する)。ready 化したら 3-0 の verify を
> もう一度通してから 3.6-2 のマージに進むこと。race 検出ロジックは CLI 側に集約されており、
> SKILL.md で SHA compare を再実装しない。
...
- **gate 10 (全 post-check stale 全網羅)** / **manual override の live 供給**: evaluator 側受け口は
  完成済み、live builder の供給 (`post_check_latest_by_expert` / `*_override`) のみ未配線。
  security / UX 単体の stale は gate 14f / 13b で検証済み。
...
- **worktree cleanup の review-glob + remote-branch 削除**: `op run worktree cleanup` は単一 apply
  ...
  (フェーズ4-4)。primitive 拡張は follow-up Issue。
