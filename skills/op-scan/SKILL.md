...
notes: v3 (2026-05-31) — ADR-0009 Phase C / C2 wave。フェーズ1 観点別並列 audit を
...
未認証の場合はユーザーに `! gh auth login` を案内して中断。
...
REGISTRY_VERIFY_JSON=$(op core registry-verify \
  ...
  --registry-path "$HOME/.claude/skills/_shared/active-expert-registry.md" \
  --planned-path "$HOME/.claude/skills/_shared/planned-experts.md" \
  2>/dev/null) || true
