...
統計的有意性 (`clear / marginal / none / unstable`) の閾値の正本は
`references/benchmark-protocol.md` の「統計的有意性の判定」節 (ここでは再定義しない)。
...
共通骨格は `_shared/project-profile.md`「対象 repo 規約への準拠 (worker 共通)」節が正本
(**撤退判断に入る前に未読なら Read する**)。撤退判断に効く optimize 固有の適用差分:

- **検証** → benchmark + 既存テスト pass が必須
- **可読性** → 可読性劣化 vs 改善幅で判断、ネスト既定値 (2 階層以内) を維持
- **デバッグ容易性優先** → unsafe / SIMD / inline asm は最終手段
...
統計判定 (`clear / marginal / none / unstable`) の算出式・閾値は
`references/benchmark-protocol.md` の「統計的有意性の判定」節が正本。以下は判定後の decision 分岐のみ:

```text
1. After ベンチマークを取った
2. 統計判定 (benchmark-protocol.md の式・閾値で clear / marginal / none / unstable を得る)

   - unstable → decision = deferred、benchmark 改善 Issue 起票
   - none     → decision = reverted、撤退
   - marginal + 高リスク → decision = reverted、撤退
   - marginal + 低リスク → decision = applied、message に marginal 旨明記
   - clear → 次へ
