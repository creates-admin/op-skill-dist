...
- OP-managed Mode: Issue コメントは書かない。完了報告の `decision` / `decision_rationale` / `baseline` / `after` / `remaining_issues` と、`needs_human_decision` / `assumptions[]` に構造化する。`escalated` では bottleneck 概要・提案する high risk 方針・リスク内容・low/medium で済む代替案を書く
- Direct Mode: 同じ内容を人間向けに提示する

## rollback

worktree 隔離なので git で戻す: 未 commit は `git restore .` (ファイル単位は `git restore <path>`)、commit 済みは `git revert <sha>`。
