...
op-architect が起票した互いに依存する工程 issue 群を、`op-depends-on` marker から作る DAG の層順に直列駆動し、
各層を op-run と同じ ClusterOrchestrator (CO) で実装する。層間に人間 gate を置き、層 N の PR が
マージされたことを確認してから次層の base を前進させる。

## 原則

1. **Direct Mode 固定** — 人間が `/op-skill:op-loop` で起動する。op-architect からの自動 handoff は持たない
2. **純 composition** — op-run skill を呼ばず、op-run skill の SKILL.md の層内手順と `cluster-orchestrator-directives.md` をそのまま適用する。
...
既定は `milestone:initial` label (op-architect の工程 issue)。`/op-skill:op-loop --label <L>` / `--numbers N M …` の明示を優先する。
...
- 推測で層順を組まない。修正後に再 `/op-skill:op-loop`。
...
`layers` を index 0 から順に、層ごとに 2-1 → 2-2 → フェーズ 3 → フェーズ 4 を行う。初期 base を確定してファイルに残す
(Bash 呼び出しをまたいで export は残らない。`_shared/bash-fence-convention.md`):

```bash
LOOP_DIR="${TMPDIR:-/tmp}/op-loop-$(date +%Y%m%d-%H%M%S)"; mkdir -p "$LOOP_DIR"
OP_RUN_BASE_REF="$(op run base-sha | jq -r '.payload.base_ref | sub("^origin/"; "")')"
...
declare -p LOOP_DIR OP_RUN_BASE_REF OP_RUN_BASE_SHA > "$LOOP_DIR/base.env"
echo "LOOP_DIR=$LOOP_DIR"
```

以降の fence は `source "<LOOP_DIR>/base.env"` で base を読む (司令官は LOOP_DIR の実パスを直書きする)。

### 2-1. 層内実装 (op-run の手順を適用)

現層の issue 集合と base を入力に、op-run skill の SKILL.md のフェーズ 1 → 2-A → 2-B → 2-Orchestrate を
...
| base SHA / ref | op-run フェーズ0-base の fence 冒頭で `source "<LOOP_DIR>/base.env"` する (設定済みの値が尊重される) |
| plan 承認 | op-run の 1-3 (plan file / ExitPlanMode) は行わない (DAG の承認と層間 gate で代替)。確定したクラスタ表は提示のみ |
| Fable escalation | op-run 1-2-g を層ごとに適用する。承認は層をまたいで引き継がない |
...
- 全 CO の verdict が `approved` / `approve_with_followup` → フェーズ 3-2 へ (follow-up は完了サマリに載せる)。
- `needs_human_decision` / `pr_open_degraded_mcp_channel` / CO の失敗がある → フェーズ 3-1 へ。
...
工程ごとの状態 (成功: PR URL / 失敗: blocker_reason) を表で示し、後続層が依存していることを伝えて選ばせる:
1. 停止 — 失敗工程を手当てしてから再 `/op-skill:op-loop` で resume (既定)
...
AskUserQuestion で詰める (op-spec skill のフェーズ 2-2〜2-3 の present → align と同じ流儀)。無ければ skip する。

- 確定内容は次層 CO spawn 時に issue の補足として渡す。
- ADR レベル (新アーキ / データモデル / depends_on 構造の変更) なら `/op-skill:op-architect` を推奨して停止する。
...
```bash
source "<LOOP_DIR>/base.env"
PREV_BASE_SHA="$OP_RUN_BASE_SHA"
git fetch origin "${OP_RUN_BASE_REF}:refs/remotes/origin/${OP_RUN_BASE_REF}"
OP_RUN_BASE_SHA="$(op run base-sha --base-ref "origin/${OP_RUN_BASE_REF}" | jq -r '.payload.base_sha')"
[ "$OP_RUN_BASE_SHA" != "$PREV_BASE_SHA" ] || { echo "⚠ origin/${OP_RUN_BASE_REF} が前進していません — マージ漏れを確認"; exit 1; }
declare -p LOOP_DIR OP_RUN_BASE_REF OP_RUN_BASE_SHA > "$LOOP_DIR/base.env"
```

前進していなければ次層に進まず人間に確認する。前進したら 2-0 の層ループに戻り次層を駆動する。

---

## フェーズ 5: resume (ステートレス)

専用 state marker は持たない。再 `/op-skill:op-loop` はフェーズ 1 から通常どおり実行し、open の工程 issue + DAG + base ref の HEAD
(`op run base-sha`) から残り層を再計算する (close 済み工程は探索から外れ、1-3 で完了扱いになる)。
...
層ごとの表 (Layer / 工程 issue / base SHA / 結果)、全工程 close 済みか、最終 base HEAD、
follow-up 候補 (approve_with_followup の `followup_findings`)、
残存事項 (3-1 で blocked にした枝 / 未完了層 — 再 `/op-skill:op-loop` で resume できる) を提示する。
