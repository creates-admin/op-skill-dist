...
- file location (移動が必要なら staged_refactor で計画化)
...
| Level | 種類 | refactor-expert の責務 |
|-------|------|----------------------|
| 0 | 静的確認 (diff / import-export / grep) | **必須** |
| 1 | 軽量静的チェック (formatter / lint / typecheck / cargo check) | **必須** |
| 2 | 変更範囲の既存テスト | **必須** (該当があれば) |
| 3 | 統合寄り smoke (Tauri command compile / export-open smoke) | 必要に応じて実施または手順提示 |
| 4 | 重い統合検証 (full build / Tauri build / E2E) | 原則、司令官判断または release / test expert に委譲 |
| 5 | 実機確認 / installer / updater | 同上 |

責務分担:

- **refactor-expert**: 自分の変更が壊していないことを **一次確認** する
- **test-expert**: 検証設計・回帰テスト化・テスト品質を担当する

詳細は `references/verification-ladder.md` を参照。
...
- `contract_preservation`: 各 boolean を全て埋める
  - `public_api_changed`
