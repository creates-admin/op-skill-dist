...
`security.source.kind: frontend_invoke` / `trust_boundary: frontend_to_backend` (対応表は本ドキュメント下部「入力源 → trust_boundary 対応表」参照)
...
`security.source.kind: config` (app 内部生成も config 系として扱う) / `trust_boundary: local_fs`
...
`security.source.kind: config` / `trust_boundary: config`
...
`security.source.kind: imported_file` / `trust_boundary: user_file`
...
`security.source.kind: env` (または `cli_arg`) / `trust_boundary: env`
