...
> `references/tauri-ipc.md` が project profile を強化する。
...
| `references/user-capability-preservation.md` | affected_user_capability / legitimate_workflow_preserved / ux_impact の判定 + apply mode の apply_allowed / requires_aux_post_check 判定マトリクス (正本) | 全 finding / apply / post-check |
...
| `references/tauri-ipc.md` | Tauri command / IPC / WebView ↔ Rust 境界 / `#[tauri::command]` 入力検証契約 / event boundary | Tauri command finding |
...
- 質問テキスト / 自由記述の "判断保留" は出さず構造化 block に閉じる。finding は静的証拠 (コード引用・呼出経路) で裏付けて報告する
...
代表例: `security:path-traversal-in-export` (file IO の path 検証漏れ) /
`security:unsafe-shell-args` (shell 引数 escape 漏れ) /
`security:secret-in-log` (log への token / secret / production path 漏洩)。

bulk_group 名の全量 (19 件) の正本は `references/source-sink-analysis.md` の bulk_group 節を参照。
