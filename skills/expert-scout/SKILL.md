...
| 6 | **marker-lint 検証 → `gh issue create`** | `_shared/expert-spawn.md (>=16)` Marker Publish Validate 節 |
...
**手順 6 (marker-lint → 起票)**:
- `gh issue create` 前に必ず `op core marker-lint --body - --source-hint issue-body --strict` で検証する
- lint が pass してから `gh issue create` を実行する
