...
notes: v2 追記 (2026-07-23, ADR-0028 第七波): §6 に `op merge verify` materials bundle の正準素材
       (gate 19 の raw body 必須性 / gate 9 の pull_request_read method / gate 21 の key 必須) を
       additive 追記。非破壊 additive のため schema_version 据え置き。
       v2 追記 (2026-07-23, ADR-0024 Phase 3 第五波 5a): §6 に PR (pull_request) の read 層実測を追記
       (pull_request_read / issue_read の PR body sanitize、search_pull_requests は raw body + labels
       同梱、PR labels の正準素材は search_pull_requests item、issue_write update は issue_number=PR番号
       でも labels 完全置換が成立)。非破壊 additive のため schema_version 据え置き。
       v2 追記 (2026-07-22, ADR-0024 Phase 3): §4 に実行者の定義を明確化 (call-spec を受けた
...
### PR (pull_request) の read 層 — sanitize と正準素材 (2026-07-23 実測、op-run Phase 3 第五波)

`mcp__github__pull_request_read` (method: get) / `issue_read` (method: get、PR 番号でも動作) は
`issue_read` の Issue body sanitize と同様、PR body の hidden marker を sanitize して返す。
一方 `mcp__github__search_pull_requests` は body を **raw のまま**返し、検索 index は HTML コメント
内の文字列もカバーする (`search_issues` と同じ挙動)。item には labels が同梱され、label 編集は
ほぼ即時に反映される。

**PR labels の素材 / read-back は `search_pull_requests` の item を正準とする**: `issue_read`
(method: get_labels) は PR 番号を解決できない (error になる)。marker 依存の PR 素材取得
(review marker 突き合わせ等) は `search_pull_requests` を使い、`issue_read` / `pull_request_read`
の body には依存しないこと (§6 冒頭の禁止事項と同じ理由)。

`issue_write` (method: update、`issue_number` に PR 番号を渡す) は PR への labels 完全置換が成立する
(Issue と同一経路が PR にも通る)。

### materials bundle (`op merge verify`) の正準素材 (2026-07-23 実測、ADR-0028 第七波)

`op merge verify` の mcp channel 対応は、司令官が MCP tool で集めた素材を 1 JSON
(materials bundle) に束ねて `--materials-json` へ注入する方式を取る (設計判断の正本は
[ADR-0028](../../op-tools/docs/adr/0028-op-merge-mcp-materials.md)、手順は
`skills/op-merge/SKILL.md` フェーズ3-0-mcp)。本節は正準素材の要点のみを additive に記す。

- **gate 19 (Fixes/Closes/Resolves 抽出) の PR body / Fixes 先 Issue body は raw body が必須**。
  `pull_request_read` / `issue_read` (method: get) の sanitize 済み body では sec/UI marker 判定が
  壊れるため、`search_pull_requests` item (PR body) / `search_issues` items (Fixes 先 Issue body)
  の raw body を使う (§6 冒頭の禁止事項と同じ理由)。
- **Refs 先 Issue は `issue_read` (get) で十分** — state 確認のみで marker 依存判定をしないため、
  sanitize の影響を受けない。
- **gate 9 (check_runs / combined_status) は `pull_request_read` の `get_check_runs` /
  `get_status` method** から取得する。
- **gate 21 (blocking-finding) は 0 件でも bundle の key を省略しない** — 「該当なし」を
  積極的に示す必要があるため、`search_issues` (label:op:blocking-finding state:open) の結果が
  空配列でも key 自体は必須 (fail-closed)。
