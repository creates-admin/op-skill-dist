...
notes: v2 (2026-07-23, ADR-0027 第六波 6b、breaking) — comment marker 群
       (`op-review-meta` / `op-review-finding` / `op-review-finding-direct` /
       `op-specialist-review-meta` / `op-review-controller-meta`) を **人間向け監査ログ専用へ降格**した。
       機械読解 (review_round 導出 / Review Fix Loop 8→2 条件判定 / post-check 結果判定 / op-merge gate 入力)
       は `<!-- op-review-state -->` (PR body の単一 JSON state 文書) が唯一の正本になった
       (ADR-0026 の `<!-- op-patrol-run -->` 降格と同型)。field 単位 schema はコメント投稿の
       検証用フォーマットとして維持するが、「機械が parse して分岐する」ロジックの参照先ではなくなった。
       `<!-- op-review-state -->` 節は「6a 予告」→「6b 正本」に昇格 (state shape の正本は
       引き続き ADR-0027 および `op-core::review_state`、本節は「PR body 上のどこに現れるか」の
       位置規約に責務を限定する点は 6a から不変)。破壊的変更のため schema_version を 1→2 に bump。
       v1 追記 (2026-07-23, ADR-0024/0027 第六波 6a) — PR body 上の `<!-- op-review-state -->`
...
> **v2 (ADR-0027 6b) 機械正本の宣言**: review lifecycle を **機械が判定・分岐するための唯一の正本**は
> PR body 上の `<!-- op-review-state -->` state 文書 (下記節、`op review state pull` で読む) である。
> 本ファイルが以下に列挙する comment marker (`op-review-meta` / `op-review-finding` /
> `op-review-finding-direct` / `op-specialist-review-meta` / `op-review-controller-meta`) は、
> PR コメントとして投稿を **維持する** (人間向け append-only 監査ログ)。field schema / enum / null
> 許可ルールはコメント投稿時の検証フォーマットとして引き続き有効だが、**op-run / op-merge のいずれも
> これらのコメントを parse して判定に使わない**。「comment marker を読む prose」は 6b で全廃した。
...
> **v2 (ADR-0027 6b): 人間向け監査ログ専用**。機械正本は `<!-- op-review-state -->` 文書の
> `attempts[]` (`op review state pull`)。op-run / op-merge はもう本 comment block を parse しない。

review-expert が **OP-managed Mode** で出力する review lifecycle metadata block。
コメントとして PR に投稿し続けるが (人間が review 履歴を追える append-only ログ)、
`review_result` 判定 / round 導出 / op-merge gate の入力は state 文書側から行う。
...
### op-merge 必須要件 (gate 3a〜3i / 5、v2: state 文書ベース)

> **v2 (ADR-0027 6b)**: 以下の要件は `<!-- op-review-state -->` 文書 (`attempts[]` の最新 entry)
> を根拠に判定する。`reviewer` / `review_comment_origin` (trusted author 相当の provenance) は
> state 文書の `attempts[].reviewer` / `attempts[].controller` field の provenance として維持し、
> 「comment 投稿者が trusted author か」の確認は行わない (ADR-0027「trust 境界の引き直し」節)。

op-merge は以下を必ず確認する。1 つでも欠けると merge 不可:

- `attempts[]` 最新 entry の `reviewer == review-expert`
- `op_run_session_id` が non-empty かつ `"unknown"` 以外
- `reviewed_head_sha == current PR head sha` (head が進んだら `pro-review-stale`)
- `review_result == approve`
- `review_round <= max_review_fix_rounds + 1` (= 3、tie-break は同 round 重複時に `reviewed_at` 最新を採用)

詳細は `skills/op-merge/SKILL.md` の review state gate 節。
...
> **v2 (ADR-0027 6b): 人間向け監査ログ専用**。機械正本は `<!-- op-review-state -->` 文書の
> `attempts[].findings[]` (`op review state pull`)。Review Fix Loop の finding 抽出は
> state 文書側から行い、本 comment block は parse しない。

review-expert が `needs-fix` / `needs-specialist-review` / `blocked` のとき、各 finding を 1 block として
コメントに残す (人間が finding 履歴を追える記録)。field schema は state 文書 `findings[]` の各要素と 1:1 対応する。
...
> **v2 (ADR-0027 6b): 人間向け監査ログ専用**。機械正本は `<!-- op-review-state -->` 文書の
> `specialist_reviews[]` (`specialist_review` payload kind、`op review state push`)。
> op-run は本 comment block を parse せず、state 文書側の判断を再委任の入力にする。
...
## `<!-- op-review-state -->` body block (機械正本、ADR-0027 6b)

> **本節は正本節 (6a の「予告のみ」から昇格)**。ADR-0027 (「review-state — PR body 型 state 文書への
> 再設計」) の 6b 全面移行 wave により、本 block が review lifecycle の **唯一の機械正本**になった。
> 上記各節の comment marker 群 (`op-review-meta` 等) は人間向け監査ログへ降格済み。

### 正本宣言

state 文書の shape (JSON schema) の正本は **ADR-0027 の「state shape (確定)」節** および
**`op-core::review_state`** (Rust 型)。本節は「PR body 上のどこに置かれるか」という位置規約と
「op-run / op-merge が読む唯一の入力である」という宣言に責務を限定し、field 単位の detailed schema は
ここに複製しない (Single Canonical Source Rule)。

### 読み書き primitive

- `op review state pull --pr N [--repo] [--input-json <search 素材>]` — state を読む。`round` の導出は
  `attempts[].review_round` の最大値。同一 round が複数存在する場合の tie-break (`reviewed_at` 最新採用)
  は CLI / op-core 側に実装され、呼び出し側 (prose) は tie-break ロジックを再実装しない。
- `op review state push --pr N --apply-json <path|-> --write-id <id> [--session S] [--input-json <search 素材>]` —
  state を書く。payload kind は `attempt` / `post_check` / `controller` / `specialist_review` /
  `manual_override` の 5 種 (1 push = 1 payload)。`write_id` は
  `${OP_RUN_SESSION_ID}-r${ROUND}-attempt` / `${OP_RUN_SESSION_ID}-postcheck-<expert>-r<N>` /
  `${OP_RUN_SESSION_ID}-terminal` 等の決定的キーとし、再送は NO_OP で安全に冪等吸収される。
  `post_checks` の expert キーは通常 `<expert 名>` だが、aux post-check (3.5-B-4 の aux ux-ui-audit-expert) は
  **`<expert>@aux`** キーとし、対応する entry の `triggered_by` を `"security-expert"` にする
  (primary/aux の map key 衝突回避規約)。
- `op review publish-approval` は approve 時の state push (attempt payload) を内包する (marker 組立 /
  marker-lint / コメント投稿 / `pro-reviewed` 付与に加え state push も 1 コマンドで atomic に行う)。
- mcp channel では pull/push とも `--input-json` (fresh `search_pull_requests` 素材) が必須。gh channel は
  fetch を内蔵するため不要。
...
既存 comment marker 群 (`op-review-meta` / `op-review-finding` / `op-review-finding-direct` /
`op-review-controller-meta` / `op-specialist-review-meta`) は **人間向け監査ログ専用へ降格**された
(本ファイル schema_version 1→2 breaking bump)。「機械が読む」ロジック (review_round 導出 /
Review Fix Loop 8→2 条件判定 / post-check 結果判定等) はすべて `<!-- op-review-state -->` body 文書側へ
移り、comment marker 群は ADR-0026 の `<!-- op-patrol-run -->` 降格と同じ扱い (parse fallback 削除、
schema は記録専用として維持) になった。

---

## 互換性 / Deprecated

v1 (2026-05-06) 時点の comment marker 群 (`op-review-meta` / `op-review-finding` /
`op-review-finding-direct` / `op-review-report` / `op-specialist-review-meta`) は v2 (ADR-0027 6b) で
**人間向け監査ログ専用へ降格**した (機械正本ではなくなった。schema / enum は監査ログ検証用に維持)。
`<!-- op-review-state -->` body block が v2 時点の唯一の機械正本 (canonical) である。
`<!-- op-review-report -->` (Direct Mode 専用) は元々 op-run / op-merge に連携しない marker のため
本降格の対象外 (従来通り Direct Mode の参考情報)。
...
> **v2 (ADR-0027 6b): 人間向け監査ログ専用**。機械正本は `<!-- op-review-state -->` 文書の
> `controller` field (`controller` payload kind、`op review state push`)。terminal state の
> 判定・後続 gate の入力は state 文書側から行う。

op-run controller が **Review Fix Loop の terminal state** (review_round 上限超過 / 強制停止)
をコメントとして残す (人間向け記録)。canonical `<!-- op-review-meta -->` (review-expert 出力) と
**独立した別 marker**。
