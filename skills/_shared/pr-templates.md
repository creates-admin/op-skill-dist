...
### body 末尾の op-review-state block (位置規約、ADR-0027 6b 機械正本)

上記テンプレの footer (`🤖 op-run による自動 PR (draft)...`) の **後** に、
`<!-- op-review-state -->` marker + JSON fence の state block が置かれる (ADR-0027)。
本 block は review lifecycle の **唯一の機械正本** (6b で確定)。本 block は **機械管理領域**であり、
人間 / agent とも手編集しない (state 文書の全置換は `op pr edit-body` / `op review state push`
経由でのみ行う)。`<!-- op-review-state -->` marker 行そのものも**機械専有**であり、人間 / agent が
PR 本文 (説明・コード例・引用) に独立行として書いてはならない — parse / splice は body 内で最初に
出現する marker 行を state block とみなすため、本文中の偽 marker は state の誤読・誤置換を招く。
field 単位の schema の正本は `skills/_shared/markers/review-markers.md` の
「`<!-- op-review-state -->` body block」節 (v2、正本節) および ADR-0027。本節が定めるのは
「PR body 上のどこに現れるか」という位置規約のみ。
...
> **本 marker は人間向け監査ログ — 機械は読まない (正本 = PR body の `<!-- op-review-state -->`、
> ADR-0027 6b)**。review_round 導出 / gate 判定 / Review Fix Loop の入力はすべて review state 文書
> (`op review state pull`) から行う。本節のコメント投稿は引き続き行うが、投稿は記録目的のみ。
...
> **本 marker は人間向け監査ログ — 機械は読まない (正本 = PR body の `<!-- op-review-state -->` の
> `specialist_reviews[]`、ADR-0027 6b)**。ClusterOrchestrator は specialist 判断確定後、本コメント投稿に
> 加えて `op review state push` (`specialist_review` payload) を必ず行い、機械分岐は state 文書を読む。
...
**op-run は本 comment ではなく state 文書 (`specialist_reviews[]`) を読み取り**、フェーズ4.5-2 の
判定優先順位 1-8 に handoff する (ADR-0027 6b)。
...
> **本 marker は人間向け監査ログ — 機械は読まない (正本 = PR body の `<!-- op-review-state -->` の
> `post_checks["ux-ui-audit-expert"]`、ADR-0027 6b)**。ClusterOrchestrator はコメント投稿に加えて
> `op review state push` (`post_check` payload) を必ず行い、op-merge gate 11-13 の判定は state 文書から行う。
...
`<!-- op-ux-ui-audit -->` マーカーで始まるコメントは人間向け監査ログとして残る。
**op-run は state 文書 (`post_checks["ux-ui-audit-expert"]`) の `post_check_result` を見て**、
BLOCK なら apply 担当 expert (designer-expert または feature-expert、すなわち該当クラスタの
op-run-expert) を再 spawn する (ADR-0027 6b)。
...
> **本 marker は人間向け監査ログ — 機械は読まない (正本 = PR body の `<!-- op-review-state -->` の
> `post_checks["security-expert"]`、ADR-0027 6b)**。ClusterOrchestrator はコメント投稿に加えて
> `op review state push` (`post_check` payload) を必ず行い、フェーズ5.5 review_mode 判定 / op-merge
> gate 14-18 の判定は state 文書から行う。aux post-check (ux-ui-audit-expert@aux) は同じ `post_checks`
> map に `<expert>@aux` キーで同居する (map key 衝突回避規約)。
