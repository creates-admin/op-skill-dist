...
- commit 後、Skill(op-skill:op-code-review) で自己検証を実施する (effort は controller 由来の `${code_review_effort}`、フェーズ3 参照)
...
> effort は controller が `model-selection.md` §5.5 で派生した `code_review_effort` を注入する
> (`auto` / 未指定なら effort 引数なしで invoke。CLI 風 flag 表記 `--high` は skill args 契約ではない。ADR-0030 CX-05)。
