...
- **配下の spawn**: Agent tool で subagent として spawn する。`subagent_type` は plugin scoped 名
  `"op-skill:<expert>"` (lookup・marker 値は bare 名)。個体名を付けない (teammate 化させない)。
  `run_in_background` は既定に委ねる。正本: `_shared/expert-spawn.md`「Plugin scoped-name 規約」「expert spawn は subagent であること」。
- **待機**: 配下の完了報告が遅れても無限待ちしない。git log / worktree / PR 状態からフェーズ完了を確認できれば
  それを根拠に次へ進む (controller の relay SendMessage に依存しない)。例外はフェーズ4 の入力条件 (fail-closed)。
...
- **label**: 遷移は CO が `op pr label-transition --pr <N> --target <review|security-post-check|ux-post-check|aux-ux-post-check> --result <...>`
...
- `fable` はこのフェーズ (と review-fix loop の再 apply) 以外に波及させない。post-check / global review は read-only のため
  各 reference の model 決定に従う (`_shared/model-selection.md` §7.2)。
...
【完了条件】
- 実装を commit する (push しない。push は ClusterOrchestrator が行う)
- commit 後に自己検証 (apply-prompt-directives.md「自己検証」節)
- 完了報告に commits_added / self_review_result / self_check_blocked を必ず含める
```
...
apply-expert がフェーズ2 の中で実施する (commit 先行: commit → `Skill(op-skill:op-code-review)` → Critical/High のみ
追加 commit して 1 回だけ再検証)。手順の正本は `references/apply-prompt-directives.md`「自己検証」節と
`_shared/apply-completion-checklist.md` Section 2-A。CO は返却値を読むだけで、指示を重ねて書かない。
...
### commit verify gate (push より前に必ず実行)
...
exit 0 以外は push しない。`blocking_reasons` 別の分岐 (UNCOMMITTED_CHANGES / COUNT_ZERO / FABRICATED_SHA /
NOT_IN_COMMIT_SET / exit 99) と retry 文面・隔離手順は `_shared/apply-completion-verify.md` §2-3 に従う。
...
(state 文書 `attempts[]` の最大 round + 1、上限 `review_round ≤ 3`)。`REVIEW_TERMINAL=1` なら
§4-2-pre-blocked の terminal 処理を行い、フェーズ5.5 / 6 を飛ばしてフェーズ8 へ (verdict `needs_human_decision`)。
...
3. §4-2-pre-2: `OP_RUN_SESSION_ID` の guard (CO 内で生成しない)
4. active lens: Round 1 は §4-2-a-pre2、Round 2+ は `references/review-fix-loop.md` §4.5-5
5. Round 2+ は fix diff のみを対象にする: `ROUND_BASE_SHA` = state 文書の前 round attempt の `reviewed_head_sha`
...
6. §4-2-a: per-phase model 解決と spawn 値の契約で review-expert を spawn (`op-skill:review-expert`)
7. §4-2-b: 返却 `reviews[]` を publish (approve = `op review publish-approval` / それ以外 = 自然文コメント + state push + label)
...
export MEDIUM_LOW_FINDINGS_SUMMARY=$(printf '%s' "$REVIEW_RESULT_JSON" | jq -r '
  [.reviews[]?.findings[]? | select(.severity | ascii_downcase | . == "medium" or . == "low")
   | "- [\(.severity)] \(.summary // .title // "finding")\(if .file then " (\(.file))" else "" end)"] | join("\n")')
...
| `needs-fix` / `needs-specialist-review` かつ `REVIEW_ROUND < 3` | `references/review-fix-loop.md` §4.5 (finding.result 主語)。再 apply → フェーズ4 の verify gate + push → 5.5 → 5 → 6 を繰り返す |
| `needs-fix` / `needs-specialist-review` かつ `REVIEW_ROUND >= 3` | terminal (`global-review-spawn.md` §4-2-pre-blocked)。verdict `needs_human_decision` (blocker_reason: review_round 上限) |
| `approve` かつ Medium/Low finding なし | verdict `approved` |
| `approve` かつ Medium/Low finding 残存 | verdict `approve_with_followup`。下記で follow-up Issue を起票 |
...
### follow-up Issue (approve_with_followup)

```bash
FOLLOWUP_BODY="$(cat <<EOF
## Follow-up: review で検出された Medium/Low finding の対応

PR ${PR_URL} の global review (round ${REVIEW_ROUND}) で検出され、PR 内では対応しなかった finding です。
...
FOLLOWUP_CREATE_JSON=$(printf '%s' "$FOLLOWUP_BODY" | op issue create \
  ...
  --body-file - --label "auto-report" --ensure-labels)
export FOLLOWUP_ISSUE_URL=$(printf '%s' "$FOLLOWUP_CREATE_JSON" | jq -r '.details.url // empty')
```

起票に失敗したら `unfiled_followup` に `{title, body, labels}` を記録する。
...
interface ClusterSummary {
  ...
  pending_label?:      string | null;   // CO が付けられなかった label
  unfiled_followup?:   { title: string; body: string; labels: string[] } | null;  // CO が起票できなかった follow-up
  degrade_note?:       string;          // pr_open_degraded_mcp_channel 時のみ
}
```

`pending_label` / `unfiled_followup` は write 失敗時のみ非 null (controller が回収する)。
...
jq -n \
  ...
  --arg followup_url "${FOLLOWUP_ISSUE_URL:-}" --arg blocker_reason "${BLOCKER_REASON:-}" \
  --arg pending_label "${PENDING_LABEL:-}" --argjson unfiled_followup "${UNFILED_FOLLOWUP_JSON:-null}" \
  --arg degrade_note "${DEGRADE_NOTE:-}" \
  'def nz: if . == "" then null else . end;
   {cluster_id: $cluster_id, pr_url: ($pr_url|nz), verdict: $verdict, round: $round,
    critical_count: $critical_count, followup_issue_url: ($followup_url|nz),
    blocker_reason: ($blocker_reason|nz), pending_label: ($pending_label|nz),
    unfiled_followup: $unfiled_followup, degrade_note: ($degrade_note|nz)}'
```
