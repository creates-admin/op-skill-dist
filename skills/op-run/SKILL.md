...
- `~/.claude/skills/_shared/apply-completion-verify.md` (>=4) — controller 側 apply 完了 verify gate の正本 (gate3 recovery 判断の receipt 仕様 / 不一致分岐 / SendMessage retry 文面 / worktrees-failed/ 隔離手順)。Phase 2-E §2-E-0 の手順詳細はこちら。**v4 の `UNCOMMITTED_CHANGES` (取りこぼし検出) を前提とする** — ClusterOrchestrator フェーズ4 の push 前 gate がこれに依存する
