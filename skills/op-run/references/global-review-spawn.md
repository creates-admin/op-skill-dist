...
notes: v3 additive (2026-06-14, Issue #723 = #682 item3 / RVW-003 解消): §4-2-a-pre2 の
...
`light-after-security-postcheck` の判定は `<!-- op-security-post-check -->` コメントが PR に存在し、
`audit_result` が `PASS` または `PASS_WITH_NOTES` であることを司令官が `op pr view --include body-comments-commits`
の `comment_details` (body + author_login + created_at、#579/#635) で確認する
(legacy skip により `audit_result: SKIPPED` の場合は `full` に倒す)。
さらに `requires_aux_post_check: true` の場合は aux ux-ui-audit-expert post-check も PASS / PASS_WITH_NOTES を確認する。
...
#### 4-2-pre. review_round の計算 (司令官側、必須)

review-expert は spawn prompt 経由で渡された `review_round` を `<!-- op-review-meta -->` の
`review_round` フィールドに転写する。司令官は spawn 前に既存の review コメントから前回 round を取得し、
インクリメントした値を prompt に渡す。テンプレ側で round を固定値として持たない (1 固定だと修正ループで毎回
round 1 が記録されてしまう問題を防ぐ)。

```bash
# === 前回 review_round の取得 (forge / 不正値を排除、PR 通算で算出) ===
# trusted author の op-review-meta コメントだけを対象にする。誰でも書ける PR コメントを
# そのまま読むと、悪意/誤投稿の偽 review_round が読み込まれて Loop 管理が壊れる。
# author filter / 値 validation の必要性は op-merge gate と同じ理由。
#
# 算出方針 (canonical): review_round は **「PR 全体での review attempt 通算」** として扱う。
# - trusted author の valid op-review-meta (reviewer == review-expert AND
#   global_review_expert == review-expert) を **head SHA 問わず全件カウント**する
# - PREV_ROUND = 過去の最大 review_round (= attempt 総数)
# - REVIEW_ROUND = PREV_ROUND + 1 (新規 attempt)
#
# rationale (head SHA filter を round 算出から外す理由):
#   - 旧設計は「reviewed_head_sha == 現在 head」だけ算入していたが、Review Fix Loop では
#     fix commit が必ず head SHA を変える → 過去 round が常に stale 扱いになり、
#     PREV_ROUND が毎回 0 になる → REVIEW_ROUND が永久に 1 のまま → max_review_fix_rounds
#     の安全弁が一度も発火しない、という致命バグになっていた
#   - 「PR 全体の attempt 通算」に統一することで、round 1 → fix → round 2 → fix → round 3
#     の正しい遷移が成立する
#   - force-push / rebase で head が大きく変わっても、同一 PR である限り attempt は累算する
#     (悪用回避: round 3 到達後の自動継続停止は op-run controller の terminal state で扱う)
#
# `reviewed_head_sha` の役割分担 (canonical):
#   - **round 算出**: 使わない (本ロジック)
#   - **op-merge stale gate**: 「最新 op-review-meta の reviewed_head_sha == current head」で merge を止める
#   - **finding 抽出 (4.5-2-pre)**: 「reviewed_head_sha == current head」で絞り、stale finding を apply に流さない
# TRUSTED_REVIEW_AUTHORS の canonical 定義 (Issue #381 解消 — SKILL.md 内唯一の定義箇所)
# フェーズ4.5-2-pre / フェーズ4.5-4 Review Fix Loop は同一 controller プロセスから
# 連続実行されるためここで初期化した変数をそのまま参照する (再定義禁止)。
...
# trusted author が PR 全体に投稿した valid op-review-meta の最大 review_round を取得する
# (head SHA は問わない。同一 PR の review attempt 通算)
...
# 数値以外 (空 / 非数値) は 0 扱いにフォールバック (forge 値を排除)
# - 「PR に valid op-review-meta が 1 件もない」場合 (= 初回 review) も 0 になる → REVIEW_ROUND=1
...
if [ "$REVIEW_ROUND" -gt "$((MAX_REVIEW_FIX_ROUNDS + 1))" ]; then
  ...
  # コメント投稿失敗は警告に留める (ラベル更新は apply_review_labels blocked で完了済み)
...
- op-review-meta / op-review-finding の **公開 field schema は不変** (review-markers.md v1 据置)。
  投稿主体のみ controller へ移管 (§4-2-b、ADR-0011 決定6)。
...
**1 回だけ**投稿する。lens 並列で agent が分裂しても複数 op-review-meta による op-merge gate 3a-3i/5 破綻を
構造排除する。投稿主体が controller 単一になるため `||fallback` で lint を握り潰さない (block なら投稿を止める、
memory `feedback_op_review_meta_reviewer_field_required`)。

**approve verdict は `op review publish-approval` (Issue #756) に集約する**。approve 時の marker 組立 /
marker-lint 自己検証 / コメント投稿 / `pro-reviewed` 付与 (= 他 review label 除去込みの atomic delta) を
1 コマンドが担うため、controller は CLI を 1 回呼ぶだけにする (bash echo 手組みの drift / 手動補完を構造排除)。
needs-fix / needs-specialist-review / blocked は finding 連番を伴うため従来どおり controller が marker を組む。
