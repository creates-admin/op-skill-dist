...
`<!-- op-review-state -->` 文書 (`op review state pull`) の最新 attempt の `review_round` と
`review_result` を組み合わせて以下のように **全体の loop continuation 可否**を判定する
(round 上限と blocked 早期停止のみ、ADR-0027 6b: state 文書ベース):
...
op-run は `<!-- op-review-state -->` 文書 (`op review state pull`) の `attempts[]` から finding を取得する。
**取得対象を最新の有効 attempt 1 件に厳格に限定する**。古い round / 別 SHA に対する finding は別 review
session のものなので、現在の Review Fix Loop の入力にしてはいけない。

#### 4.5-2-pre. finding 抽出の限定条件 (必須・stale 防止、ADR-0027 6b: state 文書ベース)

> **v2 移行 (ADR-0027 6b、破壊的簡素化)**: 旧 8 条件 AND のうち 6 条件
> (trusted author からの投稿 / `<!-- op-review-meta -->` の存在 / `reviewer == review-expert` /
> `global_review_expert == review-expert` / `review_comment_origin == "op-run"` /
> `op_run_session_id` 充足) は **state 文書の provenance (どの `write_id` / `session` で書かれたか)
> として state 文書側 (`op review state push` の入力検証・書込権限) が保証する**ため、
> Review Fix Loop の入口で再検証する必要がなくなった (「comment 投稿者が trusted author か」を
> 都度確認する trust 境界は ADR-0027「trust 境界の引き直し」節の通り「誰が PR body を編集できたか」に
> 移った)。**残る条件はこの 2 つのみ**:

```text
1. reviewed_head_sha == 現在の PR head SHA
   (current_head と異なる = stale review なので Review Fix Loop の入力に使わない)
2. review_round が attempts[] 中で最大 (= 現行の最新 review session)
   (同一 review_round が複数 attempt に重複する場合の tie-break = reviewed_at 最新採用は
    `op review state pull` / op-core 側に実装済み。本節はその結果を読むだけでよい)
```

```bash
# TRUSTED_REVIEW_AUTHORS はフェーズ4-2-pre で初期化済みだが、本節では機械判定に使わない
# (人間向け監査ログ表示にのみ使う場合があるため変数自体は保持する)。
...
# state pull: gh channel は fetch 内蔵。mcp channel は fresh search_pull_requests 素材を
# REVIEW_STATE_INPUT_JSON に用意して --input-json で渡す (github-channel.md §4)。
REVIEW_STATE_JSON=$(op review state pull --pr "$PR_NUMBER" \
  ${REVIEW_STATE_INPUT_JSON:+--input-json "$REVIEW_STATE_INPUT_JSON"})

# 最新 attempt (review_round 最大、tie-break は CLI 内蔵) を取得し、
# reviewed_head_sha == current head を確認する。
LATEST_ATTEMPT_JSON=$(printf '%s' "$REVIEW_STATE_JSON" | jq -c --arg head "$CURRENT_HEAD_SHA" '
  (.details.state.attempts // []) as $attempts
  | ($attempts | map(.review_round) | max // 0) as $max_round
  | ($attempts | map(select(.review_round == $max_round)) | sort_by(.reviewed_at) | last) as $latest
  | if ($latest == null) then {ok:false, reason:"no_attempts"}
    elif ($latest.reviewed_head_sha != $head) then {ok:false, reason:"stale_head"}
    else {ok:true, reason:"ok", attempt:$latest}
    end
')

FINDINGS_OK=$(printf '%s' "$LATEST_ATTEMPT_JSON" | jq -r '.ok')
if [ "$FINDINGS_OK" != "true" ]; then
  FINDINGS_REASON=$(printf '%s' "$LATEST_ATTEMPT_JSON" | jq -r '.reason')
  echo "⚠️  最新 review attempt が見つからないか stale です (reason=${FINDINGS_REASON}, current head=${CURRENT_HEAD_SHA:0:7})。" >&2
  echo "   stale (reviewed_head_sha != current head) の場合は再 review が必要です。" >&2
  # 通常はフェーズ4 の re-spawn ループで解消されるため、本フェーズ4.5-2 に進まない
  return 1 2>/dev/null || exit 1
fi

# $REVIEW_FINDINGS は最新 attempt の findings[] (JSON array、そのまま次節で jq 参照する)。
# 他 attempt / 他 round の finding は上記 max round + head 一致フィルタで既に除外済み。
REVIEW_FINDINGS=$(printf '%s' "$LATEST_ATTEMPT_JSON" | jq -c '.attempt.findings // []')
```

> **重要**: 上記で抽出した `$REVIEW_FINDINGS` (JSON array) のみが本 Review Fix Loop の正規入力。
> 別 round / 別 SHA の attempt に残っている古い finding は **読まない / 拾わない**。
> stale review session の指摘を現在 round に持ち越すと、修正済み issue を再修正させる
> 無限ループ / 別観点 finding の混線が起きる。

#### 4.5-2-pre-2. dispatch 用メタと handoff 用本文の分離 (必須、ADR-0027 6b: JSON 直接参照)

> **v2 移行 (ADR-0027 6b)**: `$REVIEW_FINDINGS` は awk RS 分割が必要な生テキストではなく
> **JSON array**(state 文書 `attempts[].findings[]` そのもの)。dispatch 用 / handoff 用の
> 2 系統分離という設計意図は不変だが、実装は jq の直接参照に置き換わる。
...
違うため、混ぜると「dispatch 判定に評価テキストが混入する」「prompt が冗長すぎて lens が薄まる」
の両方の事故が起きる。

```text
dispatch 用 (op-run の判定優先順位 1-8 / fallback / round 制御で参照):
- id / result / severity / lens / scope / recommended_fix_expert / requires_post_check

handoff prompt 用 (apply expert / specialist の入力にそのまま渡す):
- 上記 dispatch field + summary / file / evidence (state 文書 finding の全 field)
```

finding 単位の取り出しは jq の `select` で直接行う (awk RS 分割は不要になった):

```bash
# id ごとの dispatch メタ取得 (判定に使う field のみ)
finding_meta_for_id() {
  local target_id="$1"
  printf '%s' "$REVIEW_FINDINGS" | jq -c --arg id "$target_id" \
    '.[] | select(.id == $id) | {id, result, severity, lens, scope, recommended_fix_expert, requires_post_check}'
}

# id ごとの handoff 本文取得 (apply / specialist prompt にそのまま渡す整形テキスト)
finding_full_for_id() {
  local target_id="$1"
  printf '%s' "$REVIEW_FINDINGS" | jq -r --arg id "$target_id" '
    .[] | select(.id == $id) |
    "id: \(.id)\nresult: \(.result)\nseverity: \(.severity)\nlens: \(.lens)\nscope: \(.scope)\n" +
    "recommended_fix_expert: \(.recommended_fix_expert // "null")\nrequires_post_check: \(.requires_post_check // "null")\n\n" +
    (.summary // "") + (if .file then " (\(.file))" else "" end) +
    (if .evidence then "\n\n根拠:\n\(.evidence)" else "" end)
  '
}
```
...
`finding_meta_for_id "$RVW_ID"` の JSON をそのまま参照する。
...
「op-run: specialist 判断結果コメント (specialist expert)」節を参照、人間向け監査ログ)。
**ClusterOrchestrator は specialist の判断確定後、必ず state 文書へも push する** (ADR-0027 6b、
機械正本):

```bash
# specialist 判断確定後の state push (specialist_review payload)
SPECIALIST_PAYLOAD=$(jq -n \
  --arg source_finding_id "$RVW_ID" --arg specialist "$SPECIALIST_EXPERT" \
  --arg result "$SPECIALIST_RESULT" --arg apply_expert "${RECOMMENDED_APPLY_EXPERT:-}" \
  --arg post_check "${SPECIALIST_REQUIRES_POST_CHECK:-}" --argjson round "$REVIEW_ROUND" \
  --arg at "$(date -Iseconds)" --arg reason "$SPECIALIST_REASON" \
  '{kind:"specialist_review", value:{source_finding_id:$source_finding_id, specialist:$specialist,
    specialist_result:$result, recommended_apply_expert:(if $apply_expert=="" then null else $apply_expert end),
    requires_post_check:(if $post_check=="" then null else $post_check end),
    reviewed_round:$round, reviewed_at:$at, reason:$reason}}')
printf '%s' "$SPECIALIST_PAYLOAD" | op review state push --pr "$PR_NUMBER" \
  --apply-json - --write-id "${OP_RUN_SESSION_ID}-r${REVIEW_ROUND}-specialist-${RVW_ID}" \
  --session "$OP_RUN_SESSION_ID" \
  ${REVIEW_STATE_INPUT_JSON:+--input-json "$REVIEW_STATE_INPUT_JSON"} \
  || echo "❌ ${RVW_ID}: specialist_review の state push が失敗しました。同 PR での再委任判定が state 文書に反映されません。" >&2
```

これにより op-run の自動分岐が自然文依存にならない (機械判定は state 文書、comment は監査ログ)。
...
(a) 前 round で finding が出た lens
    - $REVIEW_FINDINGS (4.5-2-pre で抽出した state 文書最新 attempt の findings[]、ADR-0027 6b) の
      lens 表示形を内部 kebab key に逆引きして集める (例: "Security / Abuse" → security)。
...
未解決のまま approve に倒れる事故が起きる。これを防ぐため、controller は前 round の state 文書 attempt
(`$REVIEW_FINDINGS`) から **未解決の high/critical のみ**を抽出して `carryover_findings` に注入する
(ADR-0027 6b: 導出元は state 文書 `attempts[].findings[]` の `summary` / `file` / `evidence` field):

```text
carryover_findings = 前 round ($REVIEW_FINDINGS、state 文書 attempts[].findings[]) のうち、
...
(summary / file / evidence は state 文書 finding の同名 field をそのまま転記する)。
```

```bash
# carryover_findings の抽出例 (jq、$REVIEW_FINDINGS は state 文書由来の JSON array)
# $RESPAWNED_LENS_KEYS = 今 round で再 spawn する active lens (kebab key の bash 配列)
