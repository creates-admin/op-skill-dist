...
designer-expert は **作業の最初に必ず黙読する** 判断順序 (10 ステップ) + 優先順位 + 禁止事項を
`references/agent-instructions.md` に持つ (2026-07-23 に `decision-order.md` を統合、単一ファイル化)。
判断の最初に黙読し、迷ったら `agent-instructions.md` の詳細節 / `visual-quality-rubric.md` / `scan-finding-policy.md` に戻る。

### 判断装置の役割分担

designer-expert は判断順序・思想・実務原則を 2 つのファイルで持つ。

| リスト | 場所 | 何をするときに見るか | 粒度 |
|--------|------|--------------------|------|
| 判断順序 10 ステップ + 優先順位 + 「迷ったらやらない」 | `agent-instructions.md` | **作業冒頭で必ず黙読する核**、詳細展開もここ | 凝縮 + 詳細 |
| 設計原則 (思想 + 現場チェック統合) | `philosophy.md` | **設計判断の根拠**を Issue コメント等に書くとき / 現場のチェックリストとして実装中に見るとき | 哲学 + 実務 |

**実行時の最低読み込みセット**:

- scan / patrol → `agent-instructions.md` + `scan-finding-policy.md` + `visual-hierarchy-patterns.md`
- architect → `agent-instructions.md` + `project-design-system-lookup.md` + `enterprise-ui-density.md` + `visual-quality-rubric.md`
...
| File | 役割 | 読むタイミング |
|------|------|---------------|
| `references/agent-instructions.md` | 作業冒頭の核 instruction + 判断順序 10 ステップ + 優先順位 | 全フェーズの冒頭、architect / run の前 |
| `references/philosophy.md` | 設計原則 (思想 + 現場チェック統合、designer 視点) | 迷った時の立ち戻り |
...
1. `references/agent-instructions.md` を黙読 (判断順序 10 ステップを通す)
2. `references/project-design-system-lookup.md` で project 固有 DS を探す
3. `references/enterprise-ui-density.md` / `visual-hierarchy-patterns.md` で layout 設計
4. (motion を使う画面のみ) `references/motion-patterns.md` で transition / motion token を設計し、Design Plan に `### Motion Strategy` 節を追加 (AI 到達ライン ③④ は human polish / design spike を明記)
5. `references/visual-quality-rubric.md` で目標 score を宣言
