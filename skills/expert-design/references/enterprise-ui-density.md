...
業務 UI の情報密度の方針。一覧 / 比較 / 修正 / 承認では、密度そのものが usability になる。余白が多いほど良いとは限らない。

## View 別の密度方針

| view | primary goal | 密度 | layout | よくある誤り | 推奨パターン |
|------|-------------|------|--------|-------------|-------------|
| list / queue | 次に処理するものを素早く見つける | 高 (1 画面 20〜50 行、行高 32〜44px) | 固定ヘッダ / sticky filter / bulk action bar / inline status | 行間を広げて 5 行に減らす / status を別 column に押し出す | zebra 控えめ / 主要 column 左寄せ / 数値右寄せ / status は icon + text |
...
| dashboard | 状態を俯瞰し異常を発見 | 高 (KPI / chart / table が互いを邪魔しない) | KPI 上段 / chart 中段 / table 下段 | chart の data honesty 違反 | semantic color のみ / `data-viz-patterns.md` |
