...
review-expert は spawn 時に `review_round` を受け取る (1 origin)。round 1〜3 は通常の判定、
round 4 以上は規定外 spawn として即 `blocked` (本来 op-run 側 4-2-pre で停止しているはず)。
round 3 (最終許可 round) での扱いは **「round と blocked の境界 (canonical 表現)」節の注が正本**
(ここで繰り返さない)。
