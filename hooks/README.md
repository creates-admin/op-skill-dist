...
作成意図: workflow は plugin component 型に存在せず、named 解決 (Workflow({name:"op-*"})) は
         ...
         home の探索パスへ冪等 copy して既存の named 契約を無改変で成立させる (ADR-0022)。
...
`op-tools/docs/adr/0022-workflow-plugin-distribution.md`。
...
Cloud plugin に同梱される。`workflows/` も同時に同期対象へ追加済み (ADR-0022)。

## 検証待ち (ADR-0022 Proposed)

plugin SessionStart hook が **Cloud/web session で fire するか**は docs 未記載のため
実機検証で確定する (ADR-0022「検証チェックリスト」)。否認時は Workflow の `scriptPath`
経路 (Option B) へ切替える。
